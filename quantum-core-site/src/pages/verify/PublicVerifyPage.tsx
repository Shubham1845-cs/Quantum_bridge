...
export default function PublicVerifyPage() {
  ...
  const handleCopyKey = async (key: string, type: string) => {
    ...
    if (success) {
      toast.success(`${type} public key copied`);
    }
  };
        ...
        >
          <div className="text-center mb-10">
            ...
            <p className="text-white/40 text-lg">
              Independently verify post-quantum signatures
            </p>
          ...
          {error && (
            <div className="p-8 rounded-2xl bg-red-500/10 border border-red-500/30 backdrop-blur-md">
              ...
              <p className="text-red-400 text-sm text-center">
                Request not found or verification is disabled for this organization.
              </p>
            </div>
          )}
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="p-4 rounded-xl bg-black/30">
                    ...
                    <div
                      className={`text-lg font-bold ${
                        data.ecdsaVerified ? 'text-green-400' : 'text-red-400'
                      }`}
                    >
                  ...
                  <div className="p-4 rounded-xl bg-black/30">
                    ...
                    <div
                      className={`text-lg font-bold ${
                        data.dilithiumVerified ? 'text-green-400' : 'text-red-400'
                      }`}
                    >
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-white/60 text-sm">ECDSA P-256 Public Key</div>
                      <button
                        onClick={() => handleCopyKey(data.publicKeys.ecdsaPublicKey, 'ECDSA')}
                        className="px-2 py-1 text-xs text-cyber-cyan hover:text-cyber-cyan/80 transition-colors"
                      >
                        Copy
                      </button>
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <div className="text-white/60 text-sm">ML-DSA-65 Public Key</div>
                      <button
                        onClick={() =>
                          handleCopyKey(data.publicKeys.dilithiumPublicKey, 'ML-DSA')
                        }
                        className="px-2 py-1 text-xs text-cyber-cyan hover:text-cyber-cyan/80 transition-colors"
                      >
                        Copy
